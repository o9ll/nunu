...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("NUN_DEV_INSTALL") == "1"
	...
	NunFile := NunDirectory

	stat, err := os.Stat(NunFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		NunFile = path.Join(NunFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(NunFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Nun Install. Checking for hash...")

	re := regexp.MustCompile(`// Nun (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(NunDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", NunDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", NunDirectory+":", err)
		retErr = err
		return
	}
