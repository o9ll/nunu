...
var NunDirectory string

var ErrAlreadyReported = errors.New("already reported")

func init() {
	if dir := os.Getenv("NUN_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using NUN_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../NunData")
		BaseDir = path.Join(dir, "..", "NunData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Nun").UserConfig()
	}
	dir := os.Getenv("NUN_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using NUN_DIRECTORY")
		NunDirectory = dir
	} else {
		NunDirectory = path.Join(BaseDir, "nun.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, NunDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", NunDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+NunDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + NunDirectory + ": " + err.Error())
		}
	}
	return nil
}
...
func isNunLoaderAppAsar(appAsar string) (bool, error) {
...
func cleanupDesyncedPatchedInstall(dir string, isSystemElectron bool) (bool, error) {
	...
	isLoader, err := isNunLoaderAppAsar(appAsar)
	if err != nil {
		return false, err
	}
	if isLoader {
	...
	Log.Warn("Detected a patched install with a non-Nun app.asar. Discord was likely updated while patched; removing stale _app.asar")
